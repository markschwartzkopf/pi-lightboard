...
if (new Array(10).length == 10)
    format = 'dmx';
let dmxValues = new Array(513).fill(0);
...
function onDmxValues(newValues) {
    for (let x = 0; x < onScreenFaders.length; x++) {
        let fader = onScreenFaders[x];
        if (fader.type == 'dmx' &&
            newValues[fader.number] != dmxValues[fader.number]) {
            let faderElement = document.getElementById('fader-dmx-' + fader.number);
            let valElement = document.getElementById('val-dmx-' + fader.number);
...
function onDmxLabels(dmxLabels) {
    //populate DMX labels
